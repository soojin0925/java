...
public class MyThread2 {
	public static void main(String[] args) {
		...
		// main함수 안에는 기본적으로 주쓰레드(main쓰레드)가 있어 main함부수터 실행할 수 있음
		// 동시에 실행하고자하는 기능이 있다면 한명의 노동자로는 부족
		// ==> 쓰레드 추가 생성
		
		// 자바에서 쓰레드를 생성하는 방법(2): 1.Thread 상속  2.Runnable 인터페이스 지정받은 클래스
		// 추가쓰레드에서 동작 정의 : run() 메서드
		// 추가쓰레드 사용 : start() 메서드
		
		// 추가 쓰레드 생성(객체화)
		MyInput input = new MyInput();
		input.start(); 					// start() 메서드를 통해서 run() 메서드를 추가쓰레드 "중요"
